...
public class Raven : MonoBehaviour
{
    ...
    // These values below control the area the raven can move to
    ...
    private Rigidbody2D rbRaven;
    private Vector2 ravenPos;
    ...
    void Start()
    {
        ...
        rbRaven = GetComponent<Rigidbody2D>();
        isPaused = false;
        isAside = false;
    ...
    void Update()
    {
        ravenPos = (Vector2)transform.position;
        rbRaven = GetComponent<Rigidbody2D>();
        ChangeSprite();
        walkToTarget();
    }
    ...
    void walkToTarget()
    {
        ...
        if (Vector2.Distance(ravenPos, walkingTarget) > 0.1f)
        {
            // Calculate direction and apply velocity to move the raven
            Vector2 direction = (walkingTarget - ravenPos).normalized;
            rbRaven.velocity = direction * ravenSpeed; // Move the raven smoothly using velocity

        }
        else
        {
            // If the raven is close enough to the target, start the pause coroutine
            StartCoroutine(pauseRaven());
        }
    }

    private IEnumerator pauseRaven()
    {
        isPaused = true;
        rbRaven.velocity = Vector2.zero; // Stop the raven while paused
    ...
    private void ChangeSprite()
    {
        ...
        if (rbRaven.velocity.x > 0.1f)
        {
            spriteRenderer.sprite = spritesArray[3]; // Right
        }
        else if (rbRaven.velocity.x < -0.1f)
        {
